...
static int darwin_detach_kernel_driver_locked (struct libusb_device_handle *dev_handle, uint8_t interface);
...
/* must be called while holding dpriv->lock */
static int darwin_reset_device_locked (struct libusb_device_handle *dev_handle) {
  struct darwin_cached_device *dpriv = DARWIN_CACHED_DEVICE(dev_handle->dev);
  IOReturn kresult;
  int ret;
...
/* must be called while holding dpriv->lock */
static int darwin_attach_kernel_driver_locked (struct libusb_device_handle *dev_handle, uint8_t interface) {
  UNUSED(interface);
  struct darwin_cached_device *dpriv = DARWIN_CACHED_DEVICE(dev_handle->dev);
