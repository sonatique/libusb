...
/* LIBUSB_DESCRIBE is the human-readable build identifier exposed via
 * struct libusb_version::describe. version_describe.h is generated by
 * build-aux/gen-describe.sh and every supported build system invokes it
 * before compiling this translation unit. */
#include "version_describe.h"

static const struct libusb_version libusb_version_internal =
	{ LIBUSB_MAJOR, LIBUSB_MINOR, LIBUSB_MICRO, LIBUSB_NANO,
	  LIBUSB_RC, LIBUSB_DESCRIBE };
...
/* Claim an interface unless the handle holds it already. *claimed is set only
 * for a claim this call performed. The test and the claim are one critical
 * section, so *claimed holds against a concurrent claim of the same interface.
 */
...
int API_EXPORTED libusb_init_context(libusb_context **ctx, const struct libusb_init_option options[], int num_options)
{
	...
	usbi_dbg(_ctx, "libusb v%u.%u.%u.%u%s (%s)", libusb_version_internal.major, libusb_version_internal.minor,
